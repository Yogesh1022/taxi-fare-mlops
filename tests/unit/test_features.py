    ...
    return pd.DataFrame(
        {
            ...
            "tpep_dropoff_datetime": [
                base_time + timedelta(hours=i, minutes=30) for i in range(10)
            ],
...
def test_trip_duration_calculator(sample_taxi_data):
    ...
    sample_taxi_data_copy["tpep_pickup_datetime"] = pd.to_datetime(
        sample_taxi_data_copy["tpep_pickup_datetime"]
    )
    sample_taxi_data_copy["tpep_dropoff_datetime"] = pd.to_datetime(
        sample_taxi_data_copy["tpep_dropoff_datetime"]
    )
