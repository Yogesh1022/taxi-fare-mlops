...
def sample_tuning_data():
    ...
    y = pd.Series(np.random.randn(n_samples) * 10 + 20, name="target")
...
def test_tuner_initialization(train_val_test_split):
    ...
    tuner = HyperparameterTuner(
        X_train, X_val, y_train, y_val, X_test, y_test, n_trials=5, use_mlflow=False
    )
...
def test_svm_tuning(train_val_test_split):
    ...
    tuner = HyperparameterTuner(
        X_train, X_val, y_train, y_val, X_test, y_test, n_trials=3, use_mlflow=False
    )
...
def test_load_baseline_results(train_val_test_split, tmp_path):
    ...
    baseline_results = {"SVM": {"test_metrics": {"r2_score": 0.85, "rmse": 9.0, "mae": 3.5}}}
