    ...
    try:
        ...
        if not isinstance(X_train_transformed, pd.DataFrame):
            ...
            X_train_transformed = pd.DataFrame(
                X_train_transformed, columns=[f"feature_{i}" for i in range(n_features)]
            )
            X_val_transformed = pd.DataFrame(
                X_val_transformed, columns=[f"feature_{i}" for i in range(n_features)]
            )
            X_test_transformed = pd.DataFrame(
                X_test_transformed, columns=[f"feature_{i}" for i in range(n_features)]
            )
