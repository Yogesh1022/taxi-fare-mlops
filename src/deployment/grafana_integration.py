...
class ModelPerformanceMonitor:
    ...
    def record_prediction_batch(
        ...
        logger.info(
            f"[MONITOR] Accuracy: {accuracy:.1f}% | Latency: {latency_ms:.1f}ms | Throughput: {throughput}/min"
        )
    ...
    def get_performance_summary(self, lookback_hours: int = 24) -> Dict[str, Any]:
        ...
        recent = [s for s in self.snapshots if datetime.fromisoformat(s.timestamp) > cutoff_time]

        if not recent:
            return {"lookback_hours": lookback_hours, "samples": 0, "message": "No data for period"}
...
class DataDriftMonitor:
    ...
    def detect_drift(
        self, current_data: pd.DataFrame, columns: List[str] = None, psi_threshold: float = 0.1
    ) -> Dict[str, Any]:
        """
        ...
        if drift_results["psi_scores"]:
            drift_results["overall_psi"] = float(
                np.mean(list(drift_results["psi_scores"].values()))
            )
                ...
                "panels": [
                    {
                        ...
                        "targets": [{"expr": "model_accuracy_metric"}],
                        "type": "graph",
                        "gridPos": {"x": 0, "y": 0, "w": 12, "h": 8},
                    },
                    {
                        "title": "Prediction Latency (ms)",
                        "targets": [{"expr": "prediction_latency_seconds * 1000"}],
                        "type": "graph",
                        "gridPos": {"x": 12, "y": 0, "w": 12, "h": 8},
                    },
                    {
                        "title": "Throughput (samples/min)",
                        "targets": [{"expr": "throughput_per_minute"}],
                        "type": "graph",
                    ...
                    {
                        "title": "Error Rate (%)",
                        "targets": [{"expr": "error_rate_pct"}],
                        "type": "stat",
                        "gridPos": {"x": 12, "y": 16, "w": 12, "h": 8},
                        "alert": {
                            "name": "Error Rate Alert",
                            "conditions": [{"evaluator": {"type": "gt"}, "value": 5.0}],
                        },
                    },
                ],
            },
