...
class ModelOptimizer:
    ...
    def prune_model(self, threshold_percentile: float = 20.0) -> Dict[str, Any]:
        ...
        if hasattr(self.model, "coef_"):  # Linear model
            ...
            logger.info(
                f"[OPTIMIZE] Linear model: pruning {pruned}/{total} weights ({100*pruned/total:.1f}%)"
            )
            ...
            logger.info(
                f"[OPTIMIZE] Tree model: pruning {pruned}/{total} features ({100*pruned/total:.1f}%)"
            )
            pruned_pct = 100 * pruned / total
        else:
    ...
    def _generate_summary(self, output_path: Path):
        ...
        if "quantization" in self.optimization_results:
            ...
            logger.info(f"\nQuantization (int8):")
            logger.info(
                f"  Size: {quant['original_size_mb']:.2f} MB → {quant['estimated_size_mb']:.2f} MB"
            )
...
def optimize_model_for_production(
    model: Any, X_test: np.ndarray, model_name: str = "model", output_dir: str = "models"
) -> ModelOptimizer:
    """
