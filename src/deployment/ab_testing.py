...
import numpy as np
import pandas as pd
from scipy.stats import binom_test, mannwhitneyu, ttest_ind
...
class MultiArmedBandit:
    ...
    def _upper_confidence_bound(self) -> str:
        ...
        for model in self.allocations.keys():
            ...
            confidence = self.c * np.sqrt(
                np.log(sum(self.attempts.values()) + 1) / (self.attempts[model] + 1)
            )
    ...
    def get_allocations(self) -> Dict[str, float]:
        ...
        if total_rate > 0:
            self.allocations = {model: rate / total_rate for model, rate in success_rates.items()}
