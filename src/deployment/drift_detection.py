...
class DriftDetector:
    ...
    def kolmogorov_smirnov_test(self, X_current: np.ndarray) -> Dict[str, Any]:
        ...
        results = {"method": "kolmogorov_smirnov", "features_with_drift": [], "feature_stats": {}}
    ...
    def jensen_shannon_divergence(self, X_current: np.ndarray, n_bins: int = 10) -> Dict[str, Any]:
        ...
        for i, feature_name in enumerate(self.feature_names):
            ...
            feature_result = {"divergence": float(divergence), "drifted": drifted}
    ...
    def statistical_comparison(self, X_current: np.ndarray) -> Dict[str, Any]:
        ...
        for i, feature_name in enumerate(self.feature_names):
            mean_diff = (current_stats["mean"][i] - self.reference_stats["mean"][i]) / (
                self.reference_stats["std"][i] + 1e-10
            )
            ...
            if drifted:
                results["features_with_drift"].append(feature_name)
                logger.info(
                    f"[DRIFT]   {feature_name}: DRIFT DETECTED (mean_diff={mean_diff:.2f}σ, std_ratio={std_ratio:.2f})"
                )
            else:
    ...
    def run_all_tests(self, X_current: np.ndarray) -> Dict[str, Any]:
        ...
        results["drift_consensus"] = sum(
            test["overall_drift"] for test in results["tests"].values()
        )
