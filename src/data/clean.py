...
def clean_data(df, is_test=False):
    ...
    if "passenger_count" in df.columns and df["passenger_count"].isnull().any():
        ...
        logger.info(
            f"Filling {df['passenger_count'].isnull().sum()} null passenger_count with {mode_val}"
        )
    ...
    if "store_and_fwd_flag" in df.columns and df["store_and_fwd_flag"].isnull().any():
        logger.info(
            f"Filling {df['store_and_fwd_flag'].isnull().sum()} null store_and_fwd_flag with 'N'"
        )
    ...
    if "congestion_surcharge" in df.columns and df["congestion_surcharge"].isnull().any():
        logger.info(
            f"Filling {df['congestion_surcharge'].isnull().sum()} null congestion_surcharge with 0"
        )
