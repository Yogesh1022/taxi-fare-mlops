...
class GreatExpectationsFramework:
    ...
    def expect_column_values_to_not_be_null(self, column: str) -> ValidationResult:
        ...
        result = ValidationResult(
            ...
            details={'column': column, 'null_count': int(null_count), 'null_pct': float(100 * null_count / len(self.df))}
        )
        self.results.append(result)
        return result
    
    def expect_column_values_to_be_null(self, column: str, threshold_pct: float = 0) -> ValidationResult:
        ...
        result = ValidationResult(
            ...
            details={'column': column, 'null_count': int(null_count), 'null_pct': float(null_pct), 'threshold_pct': threshold_pct}
        )
        self.results.append(result)
        return result
    ...
    def expect_column_values_to_be_unique(self, column: str) -> ValidationResult:
        ...
        result = ValidationResult(
            ...
            details={'column': column, 'unique_count': int(unique_count), 'total_count': int(total_count)}
        )
        self.results.append(result)
        return result
    ...
    def expect_column_values_to_match_regex(self, column: str, regex: str) -> ValidationResult:
        ...
        result = ValidationResult(
            ...
            details={'column': column, 'regex': regex, 'matches': int(matches), 'total': int(total)}
        )
        self.results.append(result)
        return result
    ...
    def expect_column_mean_to_be_between(
        ...
        result = ValidationResult(
            ...
            details={'column': column, 'actual_mean': float(col_mean), 'min': min_value, 'max': max_value}
        )
        self.results.append(result)
        return result
    ...
    def expect_column_stdev_to_be_between(
        ...
        result = ValidationResult(
            ...
            details={'column': column, 'actual_stdev': float(col_stdev), 'min': min_value, 'max': max_value}
        )
        self.results.append(result)
        return result
    ...
    def expect_fare_distance_correlation(self, fare_col: str = 'fare_amount', dist_col: str = 'trip_distance') -> ValidationResult:
...
def validate_taxi_dataset(
    df: pd.DataFrame,
    name: str = "taxi_dataset",
    output_dir: str = "mlops/data_quality"
) -> Dict[str, Any]:
    """
