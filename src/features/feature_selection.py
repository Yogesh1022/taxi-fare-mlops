...
class FeatureSelector:
    ...
    def select_rfe(
        ...
        cv_score = cross_val_score(
            estimator, X_train[:, selected_mask], y_train, 
            cv=5, scoring='r2'
        ).mean()
    ...
    def select_lasso(
        ...
        selected_features = [self.feature_names[i] for i in range(len(self.feature_names)) if coef_mask[i]]
        
        # CV score with selected features
        cv_score = cross_val_score(
            lasso, X_scaled[:, coef_mask], y_train,
            cv=5, scoring='r2'
        ).mean()
    ...
    def select_tree_importance(
        ...
        logger.info(f"[SELECTION] Importance threshold (percentile {percentile}): {importance_threshold:.4f}")
        
        # Select features above threshold
        importance_mask = importance >= importance_threshold
        selected_features = [self.feature_names[i] for i in range(len(self.feature_names)) if importance_mask[i]]
        
        # CV score with selected features
        cv_score = cross_val_score(
            xgb, X_train[:, importance_mask], y_train,
            cv=5, scoring='r2'
        ).mean()
    ...
    def get_best_selection(self) -> Tuple[str, List[str]]:
        ...
        best_method = max(
            ...
            key=lambda x: self.selection_results[x]['cv_r2_score']
        )
